...
- 클론 직후 한 번:
```bash
git config core.autocrlf false     # 줄바꿈 꼬임 방지 (.gitattributes와 함께)
```
...
노트북마다 **한 번만**:
```bash
git config user.name "본인GitHub아이디"   # 식별자(클론 때 했으면 생략)
./sync.sh &           # 12초마다: 동기화 + 현재작업 표시 + 겹침경고 (radar/<아이디>.md 자동생성)
```
- 이후 git 명령 **손으로 칠 필요 없음.** 파일만 저장하면 데몬이 동기화.
- 새 작업 전, 내 레이더 `radar/<내 아이디>.md`의 "작업중:" 한 줄만 갱신 → 데몬이 남들과 겹치면 **🟡 경고**.
...
## 2. 일하는 법 (즉흥 모드 요약 · 상세는 `PLAYBOOK.md`)
- **방향은 하나로**: Phase 0에서 `SPINE.md` 락(핵심 전략+가정+한 방). 안 맞추면 다 어긋남.
- **마무리는 한 명**: 키잡이가 SPINE·FINAL·시간 담당. 나머지 2명은 자유.
- **세션은 5레인 장전, 필요한 것만 켬**(`LANES.md`). 레인 = 벽 아닌 출발점.
- **겹침은 시스템이 경고**: 레이더(`radar/`) + 데몬(`sync.sh`).
- **확정 즉시 손으로 전사**(안 기다림). 텍스트는 안 느리다, **손이 느리다.**
- **잘하려면**: 품질 게이트 2번(`QUALITY.md`) — SPINE 락 전 발산, 취합 전 자기채점.
...
## 3. 현장 골든룰
1. 새 작업 전 레이더 확인 → 안 겹치는 걸 집고 내 `radar/<내 아이디>.md`에 적기. `SPINE.md`는 키잡이만.
...
- `QUALITY.md` — **잘하게: 발산·레드팀·자기채점(품질 게이트 2번)** ← 점수 올리기
...
- `00_problem.md` — 문제 원문(처음 한 번, 읽기용)
