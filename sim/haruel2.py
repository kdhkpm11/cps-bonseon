...
def decide(s: Student):
    ...
    # 원칙: 사람검토는 '정서위기 + 정말 애매/왜곡'에만. 나머지는 규칙이 올바른 조치를 자동으로.
    # ① 정서위기 → 교사검토 (유일한 강제 사람개입, 안전 오버라이드)
    if flag_emotional_crisis(s):
        return ("교사검토", "정서적 상담 연계 + 학습량 일시 감축",
                "자기진단 위기신호 → 점수 무관 사람 개입(안전 오버라이드)")

    # ② 과부하(고성과+피로) → 자동추천: '감축'이 정답이라 규칙이 처리(교사검토 불필요)
    if flag_overload(s):
        return ("자동추천", "심화·증량 중단 → 학습량 감축 + 상담 안내(선택)",
                f"정답률 {acc:.0%} 높으나 피로신호 → 과부하, 증량 대신 감축(자동 교정)")

    # ③ 협업 유사 → 자동추천: 부정 자동판정을 '안 함'이 안전한 조치(반복 시에만 사람)
    if s.collab_flag:
        return ("자동추천", "부정행위 자동판정 안 함(협업 인정), 반복 시에만 교사 확인",
                "답안 유사=협업 가능 → 자동 처벌 금지(행동 의도 구분)")

    # ④ 언어장벽 → 판단보류: 능력을 왜곡할 위험 커서 사람 지원 필요(정보 불충분)
    if s.language_barrier:
        return ("판단보류", "언어 지원(문항 읽기보조) + 계산·문장형 분리 재측정",
                "문장형 저정답률=언어장벽이지 수학 부족 아님 → 자동 단정 금지")

    # ⑤ 기타 환경 프록시(접속·카메라·읽기속도) → 환경 지표 배제하고 핵심신호로 자동 처리
    if flag_context_uncertain(s):
        return ("자동추천", "표준 학습(환경 지표 미반영·맥락 보정)",
                "접속·카메라·풀이시간은 환경 프록시 → 배제, 핵심신호(정답률·오답반복)로 자동")

    # ⑥ 회피 패턴(정답률 높지만 쉬운 것만) → 자동추천: 탐색 과제로 유도(교사검토 불필요)
    if s.picks_hard_ratio is not None and s.picks_hard_ratio < 0.3 and s.picks_hard_trend < 0:
        return ("자동추천", "난이도 상향 '탐색 과제' 소량 + 성공경험 설계",
                f"정답률 {acc:.0%} 양호하나 어려운문제 선택 감소 → '문제없음' 오판 교정")
